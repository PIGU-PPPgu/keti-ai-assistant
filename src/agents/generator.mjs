...
export class GeneratorAgent extends BaseAgent {
  ...
  /**
   * @param {{ docType, params, onProgress }} task
   * onProgress(sectionId, status, score) - 进度回调
   */
  async run({ docType, params, onProgress }) {
    ...
    const results = await Promise.all(
      sections.map(async (section) => {
        ...
        const content = await this.#generateSection(section, params);
  ...
  async #generateSection(section, params) {
    ...
    return callAI(EXPERT_SYSTEM_PROMPT, userPrompt, { maxTokens: 2000 });
  }
